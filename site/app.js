...
const captainPhoto = document.querySelector("#captainPhoto");
...
async function loadMembers() {
  try {
    ...
    memberMap = new Map(rows.map((row) => {
      const [name, team, photo] = row.split(",").map((cell) => cell.trim());
      return [normalizeName(name), { name, team, photo }];
    }));
  } catch {
...
mysteryBox.addEventListener("click", () => {
  ...
  timer = setTimeout(() => {
    captainPhoto.src = `./captains/${selected.photo}`;
    captainPhoto.alt = `${selected.team} 조장 프로필`;
    resultName.textContent = selected.name;
    resultTeam.textContent = `${selected.team} 조`;
    showView("result");
  }, 1450);
});
